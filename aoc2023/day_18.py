...
def dig_out_a_hole(directions: tuple[tuple[Direction, int], ...]) -> int:
    ...
    if is_debug:
        log.debug(f"pairs {all_vertices}")
    ...
    left_pair_idx = {}
    right_pair_idx = {}
    for some_vertices in all_vertices:
        sorted_vertices = list(sorted(some_vertices))
        for idx in range(0, len(some_vertices), 2):
            (new_l_row, new_l), (new_r_row, new_r) = sorted_vertices[idx : idx + 2]
